...
class CaptionDistributor(ExsclaimTool):
    ...
    def _update_exsclaim(self,search_query,  exsclaim_dict, figure_name, delimiter, caption_dict):
        ...
        for label in caption_dict.keys():
            ...
            materials_ner_tags = caption.materials_ner(query, api)
            if not materials_ner_tags:
                materials_ner_tags = "na" 
                
            master_image = {
                "label": label,
                "description": caption_dict[label],#["description"],
                "keywords": caption.safe_summarize_caption(query , api, llm).split(', ') ,
                "context": context, #caption.get_context(query, documents,embeddings),
                "materials_ner" : materials_ner_tags
    ...
    def run(self, search_query, exsclaim_json):
        ...
        for figure_name in figures:
            ...
            try:
                ...
                api = search_query["openai_API"]
                #print('api',api)
                caption_dict = caption.safe_separate_captions(caption_text, api, llm='gpt-4-turbo-preview')# caption.associate_caption_text( # here add the gpt3 code separate_captions(caption_text) #
