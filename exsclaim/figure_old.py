...
class FigureSeparator(ExsclaimTool):
    ...
    def extract_image_objects(self, figure_path=str) -> dict:
        ...
        for result in results:
            ...
            for cls_id, box in detections_per_class.items():
                ...
                master_image_info = {
                    "classification": "subfigure",  # You might want to adjust this
                ...
                # Extract the cropped image and save it if needed
                # Create directories if they don't exist
                output_dir = self.results_directory / figure_name.replace('.', '_')
                os.makedirs(output_dir, exist_ok=True)
                
                # Crop and save the subfigure
                cropped_img = img_raw.crop((x1, y1, x2, y2))
                cropped_img.save(output_dir / f"subfig_{label}.png")

                # Add to master images list
                figure_json.setdefault("master_images", []).append(master_image_info)
    ...
    #     print('subfigure_info_before', subfigure_info)
    ...
    #     )
    #     print('subfigure_info_after labels', subfigure_info)
